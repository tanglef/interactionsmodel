...
def run_CD(X, y, Xt, yt, lambdas, eps, warmstart):
    early_stop = False
    ...
    return total, cd, early_stop, mse


def run_CBPG(X, y, Xt, yt, lambdas, eps, use_acceleration, warmstart, recompute):
    early_stop = False
    lambdas_ = [
    ...
    Xt = torch.from_numpy(Xt).contiguous().to(device).float()
    yt = torch.from_numpy(yt).view(-1, 1).to(device).float()
    ...
    for idx, pen in enumerate(lambdas_):
        ...
        res = yt - ((Xt - cbpg.meanX) / cbpg.stdX) @ cbpg.beta
        res -= cbpg.prod_Z(Xt, cbpg.theta, cbpg.meanZ, cbpg.stdZ)
        res = res.flatten()
    ...
    return total, cbpg, early_stop, mse


def update_summary(summary, total, instance, eps, name, idx, idx_ref, mse):
...
def run_all(X, y, lambdas):
    ...
    summary = {"time": [], "ratio": [], "method": [],
               "eps": [], "nnb": [], "nnt": [], "mse": []}
    idx, idx_ref = 0, 0
    for eps in [1e-3, 1e-4]:
        print(f"######## EPS = {eps} #######")
        total, cd, early_stop_cd, mse = run_CD(
            X_train, y_train, X_test, y_test, lambdas, eps, True)
        idx, idx_ref = update_summary(
            summary, total, cd, eps, "CD+warmstart", idx, idx_ref, mse
        )

        total, cbpg, early_stop, mse = run_CBPG(
            X_train, y_train, X_test, y_test, lambdas, eps,
            use_acceleration=False, warmstart=True, recompute=True
        )
        idx, idx_ref = update_summary(summary, total, cbpg, eps,
                                      "CBPG+warmstart", idx, idx_ref, mse)
    return summary
