...
def get_callback(instance):
    global obj_y
    if not type(instance.X) is torch.Tensor:
        primal = obj_y
    else:
...
if __name__ == "__main__":
    ...
    routines_names = ["CBPG_nocuda", "PGD_nocuda", "CBPG", "PGD"]
    dic_to_plot = run_full_benchmark(
        routines_names, l_lambdas=l_alpha, datatype="double"  # double to have 64
    )
    print("plotting")
    for name in routines_names:
    ...
    if save_fig:
        plt.savefig(
            os.path.join(
                path_save, f"cuda_vs_nocuda_simulated_n{n_samples}p{n_features}.pdf"
            )
        )  # noqa
    plt.show()
