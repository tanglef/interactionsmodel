...
if __name__ == "__main__":
    ...
    n_samples, n_features = 25000, 400
    ...
    routines_names = ["CBPG", "PGD"]
    dic_to_plot = run_full_benchmark(
        routines_names, l_lambdas=l_alpha, datatype="float"  # double to have 64
    )
    dic_double = run_full_benchmark(routines_names, l_alpha, datatype="double")
    print("plotting")
    for name in routines_names:
        dic_to_plot[name]["kkt"] = dic_to_plot[name]["kkt"][0][0]
        dic_to_plot[name]["times"] = dic_to_plot[name]["times"][0][0]
        dic_double[name]["kkt"] = dic_double[name]["kkt"][0][0]
        dic_double[name]["times"] = dic_double[name]["times"][0][0]

    plt.figure()
    for name in routines_names:
        plt.plot(
            dic_to_plot[name]["times"], dic_to_plot[name]["kkt"], label=name + "float32"
        )
        plt.plot(
            dic_double[name]["times"], dic_double[name]["kkt"], label=name + " float64"
        )
    ...
    if save_fig:
        plt.savefig(
            os.path.join(path_save, f"float_comparison_n{n_samples}p{n_features}.pdf")
        )  # noqa
    plt.show()
